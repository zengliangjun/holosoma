...
def _cli_impl(
    ...
    **deprecated_kwargs,
    ...
    f = tyro._cli._resolver.TypeParamResolver.resolve_params_and_aliases(f)
    ...
    backend_name = tyro._cli._settings._experimental_options["backend"]
    ...
    with tyro._cli._settings.timing_context("Generate parser specification"):
        if registry is not None:
            with registry:
                parser_spec = tyro._cli._parsers.ParserSpecification.from_callable_or_type(
                    f,
                    markers=set(),
                    description=description,
    ...
    # Initialize backend.
    if backend_name == "argparse":
        from tyro._backends._argparse_backend import ArgparseBackend
    ...
    with tyro._cli._settings.timing_context("Parsing arguments"):
    ...
    try:
        ...
        if isinstance(e.arg, tyro._cli._arguments.ArgumentDefinition):
            display_name = (
                ...
                [
                    ...
                    fmt.cols(
                        ...
                        fmt.rows(
                            ...
                            tyro._cli._arguments.generate_argument_helptext(e.arg, e.arg.lowered),
                        ),
                    ),
                ]
            )
